...
#   2. Every explicit `github:jaypetez/gbrain-copilot#<ref>` install pin in those
#      files is #latest-stable — no v-prefixed pins, no branch refs
#      (master/main). The historical un-pinned `github:jaypetez/gbrain-copilot`
...
for rel in README.md BOOTSTRAP_FOR_AGENTS.md; do
  ...
  while IFS= read -r p; do
    [ -n "$p" ] || continue
    ref="${p#github:jaypetez/gbrain-copilot#}"
    if [ "$ref" != "$SANCTIONED" ]; then
      fail=1
      echo "FAIL: $rel pins the install to '#$ref' ($p)." >&2
      echo "      The only sanctioned install pin is github:jaypetez/gbrain-copilot#$SANCTIONED." >&2
    fi
  done <<< "$pins"
done
...
# `codex plugin marketplace add jaypetez/gbrain-copilot@<ref>` in docs must use the
# release channel (@latest-stable) or the slim dist branch (@codex-plugin).
# The @-less form (`plugin marketplace add jaypetez/gbrain-copilot`) is deliberately
...
while IFS=: read -r file line match; do
  ...
  case "$ref" in
    ...
    *)
      fail=1
      echo "FAIL: $file:$line pins 'marketplace add jaypetez/gbrain-copilot@$ref' — use @latest-stable or @codex-plugin." >&2
      ;;
  esac
done < <(grep -RInE 'plugin marketplace add jaypetez/gbrain-copilot@[A-Za-z0-9._-]+' \
