...
is_allowed() {
  local f="$1"
  # plugins/gbrain/ is a verbatim payload mirror of skills/ (generated by
  # scripts/build-copilot-plugin.sh, drift-pinned by check-fork-hygiene.sh).
  # A mirrored copy inherits its source file's allowance.
  case "$f" in
    plugins/gbrain/skills/*) f="skills/${f#plugins/gbrain/skills/}" ;;
  esac
