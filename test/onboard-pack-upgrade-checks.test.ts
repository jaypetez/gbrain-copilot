...
describe('checkPackUpgradeAvailable', () => {
  it('fires on gbrain-base brain with gbrain-base-v2 available', async () => {
    ...
    // flip this assertion.
    //
    // Fork issue #5 fresh-brain noise fix: an available successor is an
    // opt-in upgrade nudge, so status is ok — the hint message +
    // manual_only remediation are the signal, not the status.
