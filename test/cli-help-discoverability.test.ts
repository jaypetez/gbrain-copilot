...
describe('issue #10 A6 — `gbrain help` alias', () => {
  test('`gbrain help` prints the same usage as `gbrain --help` and exits 0', async () => {
    const viaAlias = await help(['help']);
  ...
  test('`gbrain help <op>` routes to the per-command help', async () => {
    const { stdout, exitCode } = await help(['help', 'get']);
    expect(exitCode).toBe(0);
    expect(stdout).toContain('Usage: gbrain get <slug>');
  });

  test('`gbrain help <cli-only-cmd>` routes to that command help', async () => {
    const { stdout, exitCode } = await help(['help', 'doctor']);
    expect(exitCode).toBe(0);
    expect(stdout).toContain('Usage: gbrain doctor');
  });

  test('`gbrain help <unknown>` still fails with Unknown command', async () => {
    const { stderr, exitCode } = await help(['help', 'no-such-command-xyz']);
