...
(CLI form: `claude plugin marketplace add jaypetez/gbrain-copilot` +
`claude plugin install gbrain@gbrain-copilot`.) Prerequisites and behavior match the
[Codex plugin](CODEX.md#install-as-a-codex-plugin-recommended): the gbrain CLI
installed (`bun install -g github:jaypetez/gbrain-copilot`), a brain
