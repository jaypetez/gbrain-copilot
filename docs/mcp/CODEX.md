...
```bash
codex plugin marketplace add jaypetez/gbrain-copilot   # slim dist branch
codex plugin add gbrain@gbrain-copilot
```
...
(`bun install -g github:jaypetez/gbrain-copilot` — the npm package
...
`bun install -g github:jaypetez/gbrain-copilot` re-run refreshes the
