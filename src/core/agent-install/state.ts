...
export function writeInstallReceipt(receipt: AgentInstallReceipt): void {
  ...
  const command = helperOwned ? `bash ${shellQuote(helper)} --json`
    : `curl -fsSL https://raw.githubusercontent.com/jaypetez/gbrain-copilot/${ref}/scripts/setup-in-agent.sh | bash -s -- --root ${shellQuote(receipt.root)} --harness ${shellQuote(receipt.harness)} --json`;
