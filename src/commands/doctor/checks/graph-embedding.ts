...
export async function checkZeEmbeddingHealth(
  ...
  try {
    ...
    if (!envKey && !fileKey) {
      // Fresh-empty-brain demotion (fork issue #5): when the file config never
      // chose an embedding model, the ZE model seen here is only the gateway's
      // built-in fallback — "embedding not configured yet", the same deferred
      // posture as the embedding_disabled sentinel, not a misconfiguration.
      // Warn only when the user explicitly configured a ZeroEntropy model and
      // its key is missing.
      const explicitModel =
