...
export async function runDoctor(
  ...
  const report = outputResults(checks, jsonOutput, { engine: engine?.kind, db_url_source: dbSource ?? null });
  ...
  // Exit-code contract (fork issue #5): 0 = healthy, 1 = warnings, 2 = failures.
  // Upstream's 0/1 made warnings indistinguishable from healthy. Routed through
  // setCliExitVerdict (not process.exit) so cleanup handlers — Bun unload events,
  // open database connections — still run before the process terminates.
  setCliExitVerdict(computeExitCode(report));
}
