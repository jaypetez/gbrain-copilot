 ...
 *                  [--agent claude-code|codex|opencode|copilot|perplexity|generic]
...
export type AgentId = 'claude-code' | 'codex' | 'opencode' | 'copilot' | 'perplexity' | 'generic';
...
interface AgentSpec {
  ...
  installKind?: 'binary' | 'copilot-config' | 'opencode-json';
}

export const AGENT_IDS: AgentId[] = ['claude-code', 'codex', 'opencode', 'copilot', 'perplexity', 'generic'];

// Preserve the legacy command surface while deriving adapter facts centrally.
// installKind is the --install lane discriminator: 'binary' shells out to the
// agent's own CLI; 'opencode-json' and 'copilot-config' write the agent's MCP
// config file directly, so no binary needs to be on PATH.
export const AGENT_SPECS = Object.fromEntries(AGENT_IDS.map(id => {
  ...
  const installKind: AgentSpec['installKind'] = adapter.connection === 'copilot-json'
    ? 'copilot-config'
    : adapter.connection === 'opencode-json'
      ? 'opencode-json'
      : binary ? 'binary' : undefined;
  return [id, { id, label: adapter.label, binary, installable: adapter.connection !== 'manual', supportsOAuth: adapter.renewable, installKind }];
...
--install (claude-code, codex, opencode, copilot). The MCP URL is your remote
...
Flags:
  ...
  --agent <kind>       claude-code (default) | codex | opencode | copilot | perplexity | generic
  ...
  --install            Apply the MCP config, then smoke-test the token.
                       claude-code + codex run the agent's mcp-add command;
                       opencode + copilot write the config directly (no binary
                       needed). opencode keeps the token out of the file via
                       {env:VAR}; copilot stores the header in mcp-config.json.
...
export async function runConnect(args: string[], deps: ConnectDeps = defaultDeps): Promise<void> {
  ...
  if (!spec.installable) {
    fail(`--install supports claude-code, codex, opencode, and copilot. ${spec.label} is set up through its own UI — drop --install to print the setup steps.`);
  }
