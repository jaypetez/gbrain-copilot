...
> supported sources are `github:jaypetez/gbrain-copilot` (optionally pinned as
> `github:jaypetez/gbrain-copilot`, the form the bootstrap flow mandates) and a
...
> `codex plugin marketplace add jaypetez/gbrain-copilot` +
> `codex plugin add gbrain@gbrain-copilot` (Claude Code: `/plugin marketplace add
> garrytan/gbrain` + `/plugin install gbrain@gbrain-copilot`). Details:
